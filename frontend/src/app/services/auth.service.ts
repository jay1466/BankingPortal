...
export class AuthService {
  ...
  generateRegistrationOtp(email: string, name: string): Observable<any> {
    const body = { email: email, name: name };
    return this.http.post<any>(`${this.baseUrl}/users/register/otp`, body);
  }

  registerUser(data: any, otp: string): Observable<any> {
    return this.http.post<any>(`${this.baseUrl}/users/register?otp=${otp}`, data);
  }
