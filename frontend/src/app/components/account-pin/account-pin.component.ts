  ...
  onSubmitGeneratePIN(): void {
    if (this.pinChangeForm.valid) {
      ...
      this.apiService.createPin(newPin, password).subscribe({
        ...
        error: (error: any) => {
          this.loader.hide();
          const errorMessage = error.error?.error || error.error?.message || 'Failed to generate PIN';
  ...
  onSubmitChangePIN(): void {
    if (this.pinChangeForm.valid) {
      ...
      this.apiService.updatePin(oldPin, newPin, password).subscribe({
        ...
        error: (error: any) => {
          this.loader.hide();
          const errorMessage = error.error?.error || error.error?.message || 'Failed to update PIN';
          this._toastService.error(errorMessage);
