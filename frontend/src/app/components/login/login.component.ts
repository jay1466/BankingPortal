    ...
    if (this.loginForm.valid) {
      ...
      this.authService.login(identifier, password).subscribe({
        ...
        error: (error: any) => {
          ...
          const errorMessage = error.error?.message || error.error?.error || error.message || 'Login failed';
          this._toastService.error(typeof errorMessage === 'string' ? errorMessage : JSON.stringify(errorMessage));
