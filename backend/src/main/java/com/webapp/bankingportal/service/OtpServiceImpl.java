...
public class OtpServiceImpl implements OtpService {
    ...
    public String generateRegistrationOTP(String email) {
        ...
        OtpInfo existing = otpInfoRepository.findByAccountNumber(email);
        if (existing != null) {
            otpInfoRepository.delete(existing);
        }

        otpInfoRepository.save(new OtpInfo(email, otp, LocalDateTime.now()));
        return otp;
    }

    @Override
    public boolean validateRegistrationOTP(String email, String otp) {
        val otpInfo = otpInfoRepository.findByAccountNumberAndOtp(email, otp);
        if (otpInfo == null) {
            throw new InvalidOtpException("Invalid OTP provided");
        }

        val expired = isOtpExpired(otpInfo);
        if (!expired) {
             otpInfoRepository.delete(otpInfo); // Consume OTP
             return true;
        }
        return false;
    }
